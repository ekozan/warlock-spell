...
bundle.uploadTarball = function (version, pkg, existing, firstSnapshot, callback) {
  if (!callback) {
  ...
  function executeCreate (err) {
    ...
    spell.bundle.createTarball(process.cwd(), version, function (err, ign, filename) {
      if (err) {
        return callback(err, true)
      }

      spell.log.info('Creating snapshot ' + version.grey);
      spell.log.silly('Filename: ' + filename);
      ...
      fs.stat(filename, function (err, stat) {
        ...
        var emitter = spell.snapshots.create(pkg.name, version, filename, function (err, res) {
          if (err) {
            return callback(err);
          }
          ...
          res.on('end', function () {
            ...
            spell.log.info('Done creating snapshot ' + version.magenta);
            callback(null, version, pkg);
          });
        });

        // if (emitter && process.stdout.isTTY ) {
        //   var size;
  ...
  return [
    ...
    },
    {
      ...
      conform: function (engine) {
         if (~['node10','node9', 'php', 'ruby', 'python'].indexOf(engine)) {
            return true;
         }else{
            return false;
         }
